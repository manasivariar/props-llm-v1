...
def run_training_loop(
    ...
    overall_log_file.write("Iteration, CPU Time, API Time, Total Episodes, Total Steps, Total Reward, Predicted Reward\n")
    ...
    for episode in range(num_episodes):
        ...
        for trial_idx in range(5):
            try:
                cpu_time, api_time, total_episodes, total_steps, total_reward, pred_reward = agent.train_policy(world, curr_episode_dir)
                overall_log_file.write(f"{episode + 1}, {cpu_time}, {api_time}, {total_episodes}, {total_steps}, {total_reward}, {pred_reward}\n")
                overall_log_file.flush()
